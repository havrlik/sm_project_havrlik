...
public class SmList_1D_byte extends SmList_1D {
	
	protected SmList_1D_byte_Partial[] listAggregated;
	
	
	public SmList_1D_byte() {
		super();
	}
	
	
	public SmList_1D_byte(SmList_Size useArraySizesModel) {
		super(useArraySizesModel);
	}
	
	
	public SmList_1D_byte(int higherMediumSizeOfList) {
	...
	//@Override
	public void putValue(int rowNumber, byte value) {
	...
	//@Override
	public byte getValue(int rowNumber) {
	...
	public void clearValue(int rowNumber) {
		if (determineArrayNumberAndRowNumberInArray_notAddingArrays(rowNumber)) {
			listAggregated[arrayNumber].putValue(rowNumberInArray, (byte) 0);
		}
	}
	...
	protected void addArrayToList() {
		if (listAggregated == null || listAggregated.length == 0) {
			...
			// New list.
			listAggregated = null;
			listAggregated = new SmList_1D_byte_Partial[list_arraySizes_length];
		}
			...
			// Copy list.
			SmList_1D_byte_Partial[] list_new = new SmList_1D_byte_Partial[list_arraySizes_length];
			for (int i1 = 0; i1 < listAggregated.length; i1++) {
	...
	protected void addAnotherOneArrayToList() {
		...
		listAggregated[listAggregated_initializedQuantity] = new SmList_1D_byte_Partial(newArrayRowQuantity);
	...
	// Additional methods.
	//@Override
	public boolean is_listContainsValue(byte value) {
