...
public class SmList_2D_Gnrc_2<E> extends SmList_SteadyColumns {
	
	protected Class<E> clazz;
	protected int COLUMNSQUANTITY = 0;
	@SuppressWarnings("rawtypes")
	protected SmList_2D_Gnrc_Partial_2[] listAggregated;
	
	
	public SmList_2D_Gnrc_2(Class<E> clazz, int columnsQuantity) {
		super();
		mutualConstructor_2D(clazz, columnsQuantity);
		
		clear();
	}
	
	
	public SmList_2D_Gnrc_2(Class<E> clazz, int columnsQuantity, SmList_Size useArraySizesModel) {
	...
	public SmList_2D_Gnrc_2(Class<E> clazz, int columnsQuantity, int higherMediumSizeOfList) {
	...
	private void mutualConstructor_2D(Class<E> clazz, int columnsQuantity) {
		this.clazz = clazz;
	...
	protected void addArrayToList() {
		if (listAggregated == null || listAggregated.length == 0) {
			...
			// New list.
			listAggregated = null;
			listAggregated = new SmList_2D_Gnrc_Partial_2[list_arraySizes_length];
		}
			...
			// Copy list.
			SmList_2D_Gnrc_Partial_2[] list_new = new SmList_2D_Gnrc_Partial_2[list_arraySizes_length];
			for (int i1 = 0; i1 < listAggregated.length; i1++) {
	...
	protected void addAnotherOneArrayToList() {
		...
		listAggregated[listAggregated_initializedQuantity] = new SmList_2D_Gnrc_Partial_2(clazz, newArrayRowQuantity, COLUMNSQUANTITY);
